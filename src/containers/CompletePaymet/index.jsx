...
function CompletePayment() {
  ...
  useEffect(() => {
    ...
    console.log('[CompletePaymet] clientSecret recebido na URL:', clientSecret);

    if (!clientSecret || clientSecret === 'undefined') {
      setTimeout(() => {
        setError(
          'Pagamento não encontrado ou inválido. Volte e tente novamente.',
        );
      }, 0);
      return;
    }
    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      console.log(
        '[CompletePaymet] Resultado do retrievePaymentIntent:',
        paymentIntent,
      );
      if (!paymentIntent) {
        setError('Não foi possível recuperar o pagamento.');
        return;
      }
    ...
    return (
      <div className="container">
        <div id="payment-status">
          <div
            id="status-icon"
            style={{ backgroundColor: STATUS_CONTENT_MAP.default.iconColor }}
          >
            {STATUS_CONTENT_MAP.default.icon}
          </div>
          <h2 id="status-text">{error}</h2>
  ...
  if (status === 'succeeded') {
    ...
    statusMessage = 'Seu pagamento está sendo processado.';
  } else if (status === 'requires_payment_method') {
    statusMessage = 'Pagamento falhou, Tente Novamente! 😵';
  } else {
    statusMessage = `Status do pagamento: ${status}`;
  }
  ...
  return (
    <div className="container">
      <div id="payment-status">
        ...
        <h2 id="status-text">{statusMessage}</h2>
        {intentId && (
